...
export class Game {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    canvas.addEventListener('game:tap' as any, (e: any) => this.handleTap(e.detail.x, e.detail.y))
  ...
  update(dt: number) {
    ...
    if (this.enablePoacher && !this.poacherActive && elapsed >= 45) this.poacherActive = true
    ...
    if (this.enablePoacher && this.poacherActive) {
      ...
      if (this.poacher.pos.x < 4 || this.poacher.pos.x > this.canvas.width - 4 || this.poacher.pos.y < 4 || this.poacher.pos.y > this.canvas.height - 4) {
  ...
  handleTap(x: number, y: number) {
    ...
    if (t.type === 'trash') {
      ...
      if (!t.carried && distance(this.player.pos, t.pos) < 20) {
        t.carried = true
      } else if (t.carried && distance(this.player.pos, this.safeZone) < this.safeZone.r) {
        t.active = false
        this.addScore(20)
      }
    } else if (t.type === 'bird') {
  ...
  render() {
    ...
    for (let y = 0; y < MAP_H; y++) {
      for (let x = 0; x < MAP_W; x++) {
        ...
        if (t === 0) { g.fillStyle = '#274227' } // grass
        else if (t === 1) { g.fillStyle = '#1f351f' } // trees
        else if (t === 2) { g.fillStyle = '#266e9c' } // river
        else { g.fillStyle = '#6b5e3b' } // trail
    ...
    g.beginPath(); g.arc(this.safeZone.x, this.safeZone.y, this.safeZone.r, 0, Math.PI * 2); g.fill()
    ...
    // Poacher
    if (this.poacherActive) drawAgent(g, this.poacher.pos.x, this.poacher.pos.y, '🎒', '#d33')
  ...
  guidance() {
    ...
    if (near) {
      ...
      if (near.type === 'bird') return 'Near bird: click/tap to take photo (+15)'
    }

    // Poacher tip when active
    if (this.enablePoacher && this.poacherActive && distance(this.player.pos, this.poacher.pos) < 80) {
